...
class Method_CNN_ORL(method, nn.Module):
    ...
    def __init__(self, mName, mDescription):
        ...
        # baseline: 1 channel in, 16 channels out; 3x3 kernel w/ padding=1
        # apply convolution with 3x3 kernel/filter
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        # nonlinearity, cheap computation with ReLU
        self.relu1 = nn.ReLU()
        # pooling layer to compress information
        self.pool1 = nn.MaxPool2d(kernel_size=2)

        # SECOND CONVOLUTIONAL BLOCK
        # baseline: 16 channels in, 32 channels out; 3x3 kernel w/ padding=1
        ...
        # bottleneck features in fc layer
        self.fc1 = nn.Linear(64 * 14 * 11, 128)
        # nonlinear activation
        self.relu_fc = nn.ReLU()
        # dropout regularization
        self.dropout = nn.Dropout(0.5)
        # final prediction mapping to identity of face
    ...
    def forward(self, x):
        ...
        # classification block - flatten -> first fc layer -> dropout -> final fc layer
        x = self.flatten(x)
    ...
    def train(self, X, y):
        ...
        # enable training mode - enable dropout
        ...
        # convert x and y data to tensors and move to device
    ...
    def test(self, X):
        ...
        # convert output logits to predicted class (whichever class has highest value)
    ...
    def plot_convergence(self):
        ...
        ax1.set_title('CNN ORL Training Loss Convergence')
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Loss')
        ax1.grid(True)

        ax2.plot(epochs, self.accuracy_history, 'g-')
        ax2.set_title('CNN ORL Training Accuracy Convergence')
        ax2.set_xlabel('Epoch')
        ax2.set_ylabel('Accuracy')
        ax2.grid(True)

        plt.tight_layout()
        plt.savefig('../../result/stage_3_result/convergence_CNN_ORL.png')
        plt.show()
